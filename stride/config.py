...
logger = logging.getLogger("stride.config")
...
class Settings(BaseSettings):
    ...
    middleware: list[str] = PydanticField(
        ...
        Formatos aceitos:
        - String path: "stride.auth.AuthenticationMiddleware"
        - Shortcut: "auth", "timing", "logging", etc
        
        Exemplo:
            MIDDLEWARE='["timing", "auth", "stride.middleware.LoggingMiddleware"]'
    ...
    models_module: str = PydanticField(
        default="src.models",
        description="Módulo Python dos models (ex: 'src.models', 'myapp.models'). Usado quando installed_apps está vazio.",
    )
    ...
    admin_modules: list[str] = PydanticField(
        default=[],
        description=(
            "Módulos admin a registrar explicitamente (ex: ['apps.users.admin']). "
            "Se vazio, o admin faz scan de */admin.py no projeto."
        ),
    )
...
def _configure_auth_from_settings() -> None:
    ...
    """
    from stride.auth.base import get_auth_config, configure_auth
...
def _auto_configure_datetime(settings: Settings) -> bool:
    ...
    try:
        from stride.datetime import configure_datetime
...
# Alias for backward compatibility (tests / external code)
_preload_models_module = _auto_configure_models
...
def _auto_configure_auth(settings: Settings) -> bool:
    ...
    try:
        from stride.auth import configure_auth as _configure_auth
...
def apply_settings(
    ...
    **overrides: Any,
    ...
    """
    Single entry point to apply settings to the framework.

    Same as configure(): loads Settings (from .env + settings_class), then
    runs auto-configuration in order: datetime → models → auth → storage →
    kafka → tasks. Prefer this name when you want to "apply" config once at startup.
    """
    return configure(settings_class=settings_class, **overrides)
