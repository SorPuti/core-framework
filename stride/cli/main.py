...
Comando test:
    ...
    stride test tests/test_auth.py    # Roda arquivo específico
    stride test -v                    # Saída verbosa
    stride test --cov                 # Com cobertura de código
    stride test -k "test_login"       # Filtrar por keyword
    stride test -m unit               # Apenas testes unitários
"""
...
def load_config() -> dict[str, Any]:
    """
    Carrega configuração do projeto via Settings (settings.py + .env).
    
    Fonte única de verdade: stride.config.Settings.
    ...
    # Valores do Settings (settings.py + .env)
    config: dict[str, Any] = {
...
def _scan_for_models(root_dir: Path) -> list[str]:
    ...
    Looks for patterns like 'class X(Model)' or 'from stride.models import Model'.
    """
    model_modules = []
    ...
    for py_file in root_dir.rglob("*.py"):
        ...
        try:
            ...
            if "from stride.models import" in content or "from stride import Model" in content:
...
def _get_core_internal_models() -> list[str]:
    ...
    """
    return [
        "stride.admin.models",      # AuditLog, AdminSession, TaskExecution, etc.
        "stride.auth.models",       # User, Group, Permission (se existirem)
    ]
...
def _import_core_module(module_path: str):
    ...
    try:
        ...
        # "stride.admin.models" → "admin/models.py"
        relative = module_path.replace("stride.", "", 1).replace(".", os.sep) + ".py"
...
def discover_models(models_module: str | list[str] | None = None, rescan: bool = False) -> list[type]:
    ...
    Strategy:
    1. Always import Stride internal models (admin, auth, etc.)
    ...
    3. Otherwise, scan project recursively for Model subclasses (no cache)
    
    Args:
        models_module: Explicit module(s) to import. Can be string or list.
                      If None, auto-discovers models in the project.
        rescan: Ignored (kept for API compatibility). Discovery is always fresh.
    ...
    # ── 1. Sempre inclui os models internos do Stride ──
    # Estes módulos estão dentro do pacote stride (ex: site-packages/stride/admin/models.py)
    ...
    # Determine which modules to check (user project) - always discovery, no cache
    try:
        from stride.config import get_settings
        _s = get_settings()
        installed_apps = getattr(_s, "installed_apps", []) or []
    except Exception:
        installed_apps = []
    if installed_apps:
        ...
        # Full scan of project for models.py / Model subclasses
        print(info("Scanning project for models..."))
        modules_to_check = _scan_for_models(root_dir)
        if not modules_to_check:
            print(warning("No model modules found. Make sure your models inherit from stride.models.Model"))
        else:
            print(success(f"Found {len(modules_to_check)} model module(s)"))
...
def cmd_gentypes(args: argparse.Namespace) -> int:
    """
    Gera stubs de tipo para autocomplete no PyCharm.
    
    Este comando gera dois arquivos:
    
    1. src/_types.pyi - Stub que faz get_settings() retornar AppSettings
       Permite: settings = get_settings() com autocomplete de campos customizados
    
    2. src/_model_types.py - Tipos Literal para campos de Models
       Permite: list_display: tuple[UserField, ...] = ("id", "email")
    
    Usage:
        stride gentypes              # Gera ambos os arquivos
        stride gentypes --models     # Apenas tipos de models
        stride gentypes --settings   # Apenas stub de settings
    
    Após executar, reinicie o PyCharm para recarregar os stubs.
    """
    generate_models = args.models or (not args.settings)
    generate_settings = args.settings or (not args.models)
    
    print(bold("🔧 Stride Type Generator"))
    print()
    
    files_generated = []
    
    # ─────────────────────────────────────────────────────────────────────────
    # 1. Gerar stub para get_settings() -> AppSettings
    # ─────────────────────────────────────────────────────────────────────────
    if generate_settings:
        print(bold("Gerando stub para get_settings()..."))
        ...
        # Coleta campos da classe de settings
        settings_fields: list[tuple[str, str, Any]] = []
        
        # Campos do Pydantic model
        for field_name, field_info in settings_class.model_fields.items():
            field_type = "Any"
            annotation = field_info.annotation
            if annotation is not None:
                # Simplifica o tipo para string
            ...
            settings_fields.append((field_name, field_type, default))
        
        # Gera um módulo Python que re-exporta get_settings com tipo correto
        # Isso funciona melhor que .pyi porque PyCharm entende imports reais
        stub_lines = [
            '"""',
            'Typed re-export of get_settings() for PyCharm autocomplete.',
            '',
            'Generated by: core gentypes',
            f'Settings class: {settings_class.__module__}.{settings_class_name}',
            '',
            'USO:',
            '    # Em vez de:',
            '    from stride.config import get_settings',
            ...
            '    ',
            '    settings = get_settings()',
            f'    settings.app_name  # ✓ PyCharm sugere campos de {settings_class_name}',
            '',
            'Regenerate after changing AppSettings:',
            '    core gentypes',
            '"""',
            '',
            'from typing import TypeVar, overload, cast',
            '',
            'from stride.config import get_settings as _get_settings, Settings',
            f'from src.settings import {settings_class_name}',
            '',
            '_SettingsT = TypeVar("_SettingsT", bound=Settings)',
            '',
            '',
            '@overload',
            f'def get_settings() -> {settings_class_name}: ...',
            '',
            '',
            '@overload',
            ...
            '',
            '',
            f'def get_settings(settings_type: type[_SettingsT] | None = None) -> {settings_class_name} | _SettingsT:',
            '    """',
            '    Retorna a instância global de Settings.',
            '    ',
            f'    Retorna {settings_class_name} com todos os campos customizados.',
            '    PyCharm oferece autocomplete completo.',
            '    ',
            '    Este é um wrapper tipado de stride.config.get_settings().',
            '    """',
            '    if settings_type is not None:',
            '        return cast(_SettingsT, _get_settings(settings_type))',
            f'    return cast({settings_class_name}, _get_settings())',
            '',
            '',
            '# Re-export outras funções úteis',
            'from stride.config import (',
            '    configure,',
            '    reset_settings,',
            '    is_datetime_configured,',
            '    is_kafka_configured,',
            '    is_tasks_configured,',
            '    get_configured_subsystems,',
            ')',
            '',
            '__all__ = [',
            '    "get_settings",',
            f'    "{settings_class_name}",',
            '    "Settings",',
            '    "configure",',
            '    "reset_settings",',
            '    "is_datetime_configured",',
        ...
        # Escreve o módulo
        stub_path = Path("src/config.py")
        stub_path.parent.mkdir(parents=True, exist_ok=True)
        stub_path.write_text("\n".join(stub_lines))
        
        print(success(f"  ✓ {stub_path}"))
        print(info(f"    {settings_class_name} com {len(settings_fields)} campos"))
        files_generated.append(stub_path)
        print()
    
    # ─────────────────────────────────────────────────────────────────────────
    # 2. Gerar tipos Literal para campos de Models
    # ─────────────────────────────────────────────────────────────────────────
    if generate_models:
        print(bold("Gerando tipos Literal para Models..."))
        
        app_filter = args.app
        
        # Discover models
        try:
            from stride.db import Base
        except (ImportError, ModuleNotFoundError):
            print(warning("  stride.db não disponível - pulando geração de tipos de models"))
            print(info("  Dica: Certifique-se que o projeto tem models SQLAlchemy configurados"))
            if generate_settings and files_generated:
                # Settings foi gerado, então não é erro total
                pass
            else:
                return 1 if not files_generated else 0
            Base = None
        
        models_found: list[tuple[str, type, list[str]]] = []
        
        if Base is not None:
            # Get all registered models from Base
            for mapper in Base.registry.mappers:
                ...
                if app_filter:
                    if app_filter not in module:
                        continue
                
                # Skip internal models
                if module.startswith("stride.admin.models") or module.startswith("stride.auth.models"):
                ...
                try:
                    ...
                    models_found.append((model_name, model_class, columns))
                except Exception as e:
                    print(warning(f"  Skipping {model_name}: {e}"))
                    continue
        ...
        if models_found:
            ...
            lines = [
                '"""',
                'Auto-generated Literal types for Model fields.',
                '',
                'Generated by: core gentypes',
                'Purpose: Enable PyCharm autocomplete for ModelAdmin field references.',
                '',
                'Usage:',
                '    from src._model_types import UserField, DomainField',
                '    ',
                '    class UserAdmin(ModelAdmin[User]):',
                '        list_display: tuple[UserField, ...] = ("id", "email")',
                '        search_fields: tuple[UserField, ...] = ("email",)',
                '',
                'Regenerate after adding/removing model fields:',
                '    core gentypes',
                '"""',
                '',
            ...
            for model_name, model_class, columns in models_found:
                # Format columns as Literal
                cols_str = ", ".join(f'"{c}"' for c in columns)
                type_name = f"{model_name}Field"
                
                lines.append(f"# {model_name} ({model_class.__module__})")
                lines.append(f"{type_name} = Literal[{cols_str}]")
                lines.append("")
                
                print(success(f"  ✓ {model_name}") + f" ({len(columns)} fields)")
            
            # Add __all__
            all_types = [f"{m[0]}Field" for m in models_found]
            lines.append("")
            ...
            print()
            print(success(f"  ✓ {output_path}"))
            print(info(f"    {len(models_found)} models, {sum(len(m[2]) for m in models_found)} total fields"))
            files_generated.append(output_path)
        else:
            print(warning("  No models found."))
            if app_filter:
                print(info(f"  Tip: Check if app '{app_filter}' has models registered."))
    ...
    print()
    if files_generated:
        print(success("✓ Tipos gerados com sucesso!"))
        print()
        print(bold("Uso:"))
        print()
        print("  # get_settings() com autocomplete de AppSettings")
        print("  from src.config import get_settings  # ← use src.config")
        print("  settings = get_settings()")
        print("  settings.app_name  # ✓ Autocomplete funciona")
        print()
        print("  # Campos de Model com autocomplete")
        print("  from src._model_types import UserField")
        print('  list_display: tuple[UserField, ...] = ("id", "email")')
        print()
        print(info("Dica: Use 'from src.config import get_settings' para autocomplete."))
    else:
        print(warning("Nenhum arquivo gerado."))
        return 1
...
def show_template_menu() -> str | None:
    ...
    from stride.cli.templates import list_available_templates, get_template_metadata
    
    # Try interactive menu first
    try:
        from stride.cli.interactive import InteractiveMenu, is_interactive
...
def show_python_menu() -> str | None:
    ...
    try:
        from stride.cli.interactive import InteractiveInput, get_installed_python_versions, get_available_python_versions, is_interactive
...
def cmd_init_from_templates(project_name: str, python_version: str, skip_venv: bool, template_name: str | None = None) -> int:
    ...
    """
    import subprocess
    from stride.cli.templates import load_all_templates, get_template_dirs, get_template_metadata
    ...
    step += 1
    print(f"  {step}. stride migrate")
    step += 1
    print(f"  {step}. stride run")
...
def cmd_templates(args: argparse.Namespace) -> int:
    ...
    from stride.cli.templates import get_all_templates_metadata
    ...
    print("\n" + "-" * 65)
    print(f"\n{info('Usage:')}")
    print(f"  stride init myproject --template <name>")
    print(f"  stride init myproject  # Interactive menu")
    print()
...
def cmd_init(args: argparse.Namespace) -> int:
    ...
    if not has_explicit_template and not has_explicit_name and not has_explicit_python:
        try:
            from stride.cli.interactive import interactive_project_setup, is_interactive
...
def cmd_makemigrations(args: argparse.Namespace) -> int:
    ...
    if not models and not args.empty:
        print(warning("No models found."))
        print(info("Tip: Make sure your models inherit from stride.models.Model"))
        print(info("     Or set 'models_module' in Settings (settings.py / .env)"))
        return 1
    
    print(f"  Found {len(models)} model(s): {', '.join(m.__name__ for m in models)}")
    
    # Executa makemigrations
    from stride.migrations import makemigrations
...
def cmd_showmigrations(args: argparse.Namespace) -> int:
    ...
    from stride.migrations import showmigrations
...
def cmd_rollback(args: argparse.Namespace) -> int:
    ...
    from stride.migrations import rollback
...
def cmd_dbinfo(args: argparse.Namespace) -> int:
    ...
    from stride.migrations.cli import dbinfo
...
def cmd_reset_db(args: argparse.Namespace) -> int:
    ...
    async def reset():
        ...
        print(info("  2. stride migrate"))
        print(info("  3. stride run"))
        return 0
...
def cmd_shell(args: argparse.Namespace) -> int:
    ...
    async def init_db():
        from stride.models import init_database, get_session
...
from stride.dependencies import DatabaseSession
...
def cmd_worker(args: argparse.Namespace) -> int:
    ...
    # Run worker
    from stride.tasks.worker import run_worker
...
def cmd_scheduler(args: argparse.Namespace) -> int:
    ...
    # Run scheduler
    from stride.tasks.scheduler import run_scheduler
...
def cmd_consumer(args: argparse.Namespace) -> int:
    ...
    async def run_consumer():
        from stride.messaging.kafka import KafkaConsumer
        from stride.messaging.registry import get_consumer, get_consumers
...
def cmd_runworker(args: argparse.Namespace) -> int:
    ...
    from stride.messaging.workers import get_worker, list_workers, run_worker, run_all_workers
...
def _discover_and_import_workers() -> list[str]:
    ...
    Searches for:
    1. Settings.workers_module (settings.py / .env)
...
def cmd_workers_list(args: argparse.Namespace) -> int:
    ...
    from stride.messaging.workers import get_all_workers
    ...
    if not workers:
        ...
        print("  - workers_module in Settings (settings.py / .env)")
        ...
        print("  from stride.messaging import worker")
        print()
...
def cmd_topics_list(args: argparse.Namespace) -> int:
    ...
    async def list_topics():
        ...
        if kafka_backend == "confluent":
            ...
            from stride.messaging.kafka import KafkaAdmin
...
def cmd_createsuperuser(args: argparse.Namespace) -> int:
    ...
    if not asyncio.run(check_tables_exist()):
        ...
        print(f"  {bold('stride makemigrations --name initial')}")
        print(f"  {bold('stride migrate')}")
        print()
        return 1
    ...
    # ── Resolve User model ──────────────────────────────────────────────
    try:
        from stride.auth.models import get_user_model
        User = get_user_model()
    except RuntimeError:
        try:
            ...
            print("  from stride.auth import AbstractUser")
            print()
            ...
            print(f"  {bold('stride init myproject --template default')}")
            print()
            return 1
...
def _discover_and_import_tasks() -> list[str]:
    ...
    Searches for:
    1. Settings.tasks_module (settings.py / .env)
...
def cmd_tasks(args: argparse.Namespace) -> int:
    ...
    from stride.tasks.registry import list_tasks
    ...
    if not tasks:
        ...
        print("  - tasks_module in Settings (settings.py / .env)")
        ...
        print()
        print("  # src/tasks.py")
        print("  from stride.tasks import task, periodic_task")
        print()
...
def create_parser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(
        prog="stride",
        description="Stride CLI - Django-inspired, FastAPI-powered",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  stride init myproject          Create a new project
  stride makemigrations          Generate migrations
  stride migrate                 Apply migrations
  stride run                     Start development server
  stride shell                   Open interactive shell

For more information, visit: https://github.com/SorPuti/stride
        """,
    )
    ...
    collectstatic_parser.add_argument(
        "-o", "--output",
        help="Output directory (default: ./static/stride-admin)"
    )
    collectstatic_parser.add_argument(
        "--no-hash",
        action="store_true",
    ...
    # gentypes
    gentypes_parser = subparsers.add_parser(
        "gentypes",
        help="Generate type stubs for PyCharm autocomplete (Settings + Models)"
    )
    gentypes_parser.add_argument(
        "-o", "--output",
        default="src/_model_types.py",
        help="Output file for model types (default: src/_model_types.py)"
    )
    gentypes_parser.add_argument(
        "-a", "--app",
        help="Filter models by app name (e.g., 'users', 'domains')"
    )
    gentypes_parser.add_argument(
        "--include-internal",
        action="store_true",
        help="Include internal framework models (auth, admin)"
    )
    gentypes_parser.add_argument(
        "--models",
        action="store_true",
        help="Generate only model field types (skip settings stub)"
    )
    gentypes_parser.add_argument(
        "--settings",
        action="store_true",
        help="Generate only settings stub (skip model types)"
    )
    gentypes_parser.set_defaults(func=cmd_gentypes)
