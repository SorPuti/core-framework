...
if TYPE_CHECKING:
    from stride.messaging.base import MessageBroker, Producer, Consumer, EventHandler
...
def register_broker(broker: "MessageBroker", default: bool = False) -> None:
    ...
    Example:
        from stride.messaging.kafka import KafkaBroker
...
def get_broker(name: str | None = None) -> "MessageBroker":
    ...
    if broker_name is None or broker_name not in _brokers:
        ...
        if kafka_backend == "confluent":
            from stride.messaging.confluent import ConfluentBroker
...
def get_kafka_consumer_class() -> type:
    ...
    if kafka_backend == "confluent":
        from stride.messaging.confluent import ConfluentConsumer
...
async def publish(
    ...
    from stride.messaging.topics import Topic
...
async def publish_event(
    ...
    """
    from stride.messaging.base import Event
    from stride.config import get_settings
