...
from stride.datetime import timezone, DateTime

if TYPE_CHECKING:
    from stride.querysets import QuerySet, SoftDeleteQuerySet, TenantSoftDeleteQuerySet
...
class Field:
    ...
    long_text = text  # Alias; prefer Field.text()
    ...
    def slug(
        *,
        ...
        """Campo slug (string com unique e index por padrão)."""
        return mapped_column(
            String(max_length),
            nullable=nullable,
            default=default,
    ...
    def email(
        *,
        ...
        """Campo email (string indexada por padrão)."""
        return mapped_column(
            String(max_length),
            nullable=nullable,
            default=default,
    ...
    def struct(
        ...
        *,
        ...
        """
        Campo para schemas estruturados (StructSchema).
        
        Permite definir estruturas JSON complexas com validação automática,
        acesso por atributos tipados e migração automática.
        
        Args:
            schema_class: Classe StructSchema definindo a estrutura
            default: Valor default (dict ou instância StructSchema)
            nullable: Se o campo pode ser NULL
            index: Se deve criar índice GIN no PostgreSQL
        
        Example:
            from stride.schema import StructSchema, StringField, BooleanField, NestedField
            ...
            user.preferences.theme = "dark"  # Validates choices!
            
            # Automatic migration
            # Old data: {"lang": "en"} → maps to language via alias
            # Missing fields → use defaults
            # Unknown fields → preserved in _extra_data
        
        Note:
            - Usa JSONB no PostgreSQL (indexável)
            - Lazy loading com cache
            - Migração automática via aliases
            - Preserva campos desconhecidos
        """
        from stride.fields import AdaptiveJSON
        from stride.schema import StructSchema, StructDescriptor
        
        # Handle default value
        def default_factory():
            if default is None:
                # Create default instance from schema
                return schema_class.default_dict()
            if isinstance(default, dict):
                return default
            if isinstance(default, StructSchema):
                return default.to_dict()
            return default
        
        # Create the column with JSON type
        column = mapped_column(
            AdaptiveJSON,
            nullable=nullable,
...
class Manager[T: "Model"]:
    ...
    def _create_queryset(self) -> "QuerySet[T]":
        """Cria o QuerySet usado pelos métodos de query. Subclasses override para retornar QuerySet customizado."""
        from stride.querysets import QuerySet
        return QuerySet(self._model_class, self._session)
    
    # Query methods
    def filter(self, **kwargs: Any) -> "QuerySet[T]":
        """Filtra registros por condições."""
        return self._create_queryset().filter(**kwargs)
    
    def exclude(self, **kwargs: Any) -> "QuerySet[T]":
        """Exclui registros por condições."""
        return self._create_queryset().exclude(**kwargs)
    
    def order_by(self, *fields: str) -> "QuerySet[T]":
        """Ordena resultados."""
        return self._create_queryset().order_by(*fields)
    
    def limit(self, value: int) -> "QuerySet[T]":
        """Limita o número de resultados."""
        return self._create_queryset().limit(value)
    
    def offset(self, value: int) -> "QuerySet[T]":
        """Define o offset dos resultados."""
        return self._create_queryset().offset(value)
    
    async def all(self) -> Sequence[T]:
        """Retorna todos os registros."""
        return await self._create_queryset().all()
    
    async def get(self, **kwargs: Any) -> T:
        """Retorna um único registro ou levanta exceção."""
        return await self._create_queryset().filter(**kwargs).get()
    
    async def get_or_none(self, **kwargs: Any) -> T | None:
        """Retorna um único registro ou None."""
        return await self._create_queryset().filter(**kwargs).first()
    
    async def first(self) -> T | None:
        """Retorna o primeiro registro ou None."""
        return await self._create_queryset().first()
    
    async def count(self) -> int:
        """Conta registros."""
        return await self._create_queryset().count()
    ...
    async def last(self) -> T | None:
        ...
        return await self._create_queryset().last()
    
    async def values(self, *fields: str) -> list[dict[str, Any]]:
        """Retorna dicionários com os campos especificados. Se nenhum campo, retorna todos."""
        return await self._create_queryset().values(*fields)
    
    async def values_list(self, *fields: str, flat: bool = False) -> list[Any]:
        """Retorna tuplas com os valores. Se flat=True e um campo, retorna lista simples."""
        return await self._create_queryset().values_list(*fields, flat=flat)
    
    async def aggregate(self, **kwargs: Any) -> dict[str, Any]:
        """Executa funções de agregação (Count, Sum, Avg, etc.)."""
        return await self._create_queryset().aggregate(**kwargs)
    
    def select_related(self, *fields: str) -> "QuerySet[T]":
        """Carrega relacionamentos junto com a query principal (JOIN)."""
        return self._create_queryset().select_related(*fields)
    
    def prefetch_related(self, *fields: str) -> "QuerySet[T]":
        """Pré-carrega relacionamentos em queries separadas."""
        return self._create_queryset().prefetch_related(*fields)
...
async def create_tables() -> None:
    ...
    """
    import logging
    _log = logging.getLogger("stride.database")
...
class SoftDeleteManager[T: "Model"](Manager[T]):
    ...
    def _create_queryset(self) -> "SoftDeleteQuerySet[T]":
        ...
        from stride.querysets import SoftDeleteQuerySet
    ...
    def using(self, session: AsyncSession) -> "SoftDeleteManager[T]":
        """Define a sessão; retorna novo SoftDeleteManager preservando deleted_field."""
        new_manager = SoftDeleteManager(self._model_class, self._deleted_field)
        new_manager._session = session
        return new_manager
...
class TenantSoftDeleteManager[T: "Model"](SoftDeleteManager[T]):
    ...
    def _create_queryset(self) -> "TenantSoftDeleteQuerySet[T]":
        ...
        from stride.querysets import TenantSoftDeleteQuerySet
    ...
    def using(self, session: AsyncSession) -> "TenantSoftDeleteManager[T]":
        """Define a sessão; retorna novo TenantSoftDeleteManager preservando campos."""
        new_manager = TenantSoftDeleteManager(
            self._model_class, self._deleted_field, self._tenant_field
        )
        new_manager._session = session
        return new_manager
