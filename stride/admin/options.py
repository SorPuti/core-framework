...
from stride.admin.exceptions import AdminRegistrationError
from stride.admin.types import (
    ModelT,
    WidgetConfig,
    FieldsetConfig,
    IconType,
...
logger = logging.getLogger("stride.admin")
...
class ModelAdmin(Generic[ModelT]):
    ...
    def get_actions_metadata(self) -> list[ActionInfo]:
        ...
        """
        from stride.admin.types import ActionInfo
    ...
    def get_column_info(self) -> list[dict[str, Any]]:
        ...
        try:
            for col in self.model.__table__.columns:
                ...
                # ── StructSchema detection ──
                struct_schema = getattr(col, "info", {}).get("struct_schema")
                struct_schema_info = None
                if struct_schema:
                    # This is a StructSchema column - use struct_editor widget
                    struct_schema_info = {
                        "name": struct_schema.__name__,
                        "fields": {
                            name: field.to_dict()
                            for name, field in struct_schema._fields.items()
                        },
                    }
    ...
    def get_storage_file_fields(self) -> list[str]:
        ...
        """
        try:
            from stride.storage import get_storage_file_fields as _get_storage_file_fields
