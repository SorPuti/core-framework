...
def _scan_for_models(root_dir: Path) -> list[str]:
    ...
    Looks for patterns like 'class X(Model)' or 'from strider.models import Model'.
    """
    model_modules = []
...
def _get_core_internal_models() -> list[str]:
    ...
    """
    return [
        "strider.admin.models",      # AuditLog, AdminSession, TaskExecution, etc.
        "strider.auth.models",       # User, Group, Permission (se existirem)
    ]
...
def _import_core_module(module_path: str):
    ...
    try:
        ...
        relative = module_path.replace("strider.", "", 1).replace(".", os.sep) + ".py"
...
def discover_models(models_module: str | list[str] | None = None, rescan: bool = False) -> list[type]:
    ...
    if installed_apps:
        ...
        if not modules_to_check:
            print(warning("No model modules found. Make sure your models inherit from strider.models.Model"))
        else:
...
def cmd_gentypes(args: argparse.Namespace) -> int:
    ...
    if generate_settings:
        ...
        stub_lines = [
            ...
            '',
            'from strider.config import get_settings as _get_settings, Settings',
    ...
    if generate_models:
        ...
        if Base is not None:
            ...
            for mapper in Base.registry.mappers:
                ...
                if module.startswith("strider.admin.models") or module.startswith("strider.auth.models"):
...
def cmd_test(args: argparse.Namespace) -> int:
    ...
    print(bold("Strider Test Runner"))
    print()
...
def show_template_menu() -> str | None:
    ...
    from strider.cli.templates import list_available_templates, get_template_metadata
    
    # Try interactive menu first
    try:
        from strider.cli.interactive import InteractiveMenu, is_interactive
...
def show_python_menu() -> str | None:
    ...
    try:
        from strider.cli.interactive import InteractiveInput, get_installed_python_versions, get_available_python_versions, is_interactive
...
def cmd_init_from_templates(project_name: str, python_version: str, skip_venv: bool, template_name: str | None = None) -> int:
    ...
    """
    import subprocess
    from strider.cli.templates import load_all_templates, get_template_dirs, get_template_metadata
    ...
    print(f"  {step}. strider makemigrations --name initial")
    step += 1
    print(f"  {step}. strider migrate")
    step += 1
    print(f"  {step}. strider run")
...
def cmd_templates(args: argparse.Namespace) -> int:
    ...
    from strider.cli.templates import get_all_templates_metadata
...
def cmd_init(args: argparse.Namespace) -> int:
    ...
    if not has_explicit_template and not has_explicit_name and not has_explicit_python:
        try:
            from strider.cli.interactive import interactive_project_setup, is_interactive
...
def cmd_makemigrations(args: argparse.Namespace) -> int:
    ...
    if not models and not args.empty:
        print(warning("No models found."))
        print(info("Tip: Make sure your models inherit from strider.models.Model"))
    ...
    from strider.migrations import makemigrations
...
def cmd_showmigrations(args: argparse.Namespace) -> int:
    ...
    from strider.migrations import showmigrations
...
def cmd_rollback(args: argparse.Namespace) -> int:
    ...
    from strider.migrations import rollback
...
def cmd_dbinfo(args: argparse.Namespace) -> int:
    ...
    from strider.migrations.cli import dbinfo
...
def cmd_check(args: argparse.Namespace) -> int:
    ...
    from strider.migrations.engine import MigrationEngine
    from strider.migrations.analyzer import (
        MigrationAnalyzer,
...
def cmd_reset_db(args: argparse.Namespace) -> int:
    ...
    async def reset():
        ...
        print(info("  1. strider makemigrations --name initial"))
        print(info("  2. strider migrate"))
        print(info("  3. strider run"))
        return 0
...
def cmd_shell(args: argparse.Namespace) -> int:
    ...
    print(info("Strider Interactive Shell"))
    ...
    async def init_db():
        from strider.models import init_database, get_session
...
def cmd_runworker(args: argparse.Namespace) -> int:
    ...
    from strider.messaging.workers import get_worker, list_workers, run_worker, run_all_workers
...
def cmd_workers_list(args: argparse.Namespace) -> int:
    ...
    from strider.messaging.workers import get_all_workers
    ...
    if not workers:
        ...
        print("  from strider.messaging import worker")
        print()
...
def cmd_topics_list(args: argparse.Namespace) -> int:
    ...
    async def list_topics():
        ...
        if kafka_backend == "confluent":
            ...
            from strider.messaging.kafka import KafkaAdmin
...
def cmd_createsuperuser(args: argparse.Namespace) -> int:
    ...
    if not asyncio.run(check_tables_exist()):
        ...
        print(f"  {bold('strider makemigrations --name initial')}")
        print(f"  {bold('strider migrate')}")
        print()
        return 1
...
def cmd_tasks(args: argparse.Namespace) -> int:
    ...
    if not tasks:
        ...
        print()
        print("  # src/tasks.py")
        print("  from strider.tasks import task, periodic_task")
        print()
...
def cmd_collectpermissions(args: argparse.Namespace) -> int:
    ...
    async def run():
        ...
        from strider.models import init_database, get_session
        from strider.auth.models import Permission
...
def create_parser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(
        prog="strider",
        description="Strider CLI - Django-inspired, FastAPI-powered",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  strider init myproject          Create a new project
  strider makemigrations          Generate migrations
  strider migrate                 Apply migrations
  strider run                     Start development server
  strider shell                   Open interactive shell

For more information, visit: https://github.com/SorPuti/strider
        """,
    )
