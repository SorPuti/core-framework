...
if TYPE_CHECKING:
    from strider.messaging.base import MessageBroker, Producer, Consumer, EventHandler
...
def register_broker(broker: "MessageBroker", default: bool = False) -> None:
    ...
    Example:
        from strider.messaging.kafka import KafkaBroker
...
def get_broker(name: str | None = None) -> "MessageBroker":
    ...
    if broker_name is None or broker_name not in _brokers:
        ...
        if kafka_backend == "confluent":
            from strider.messaging.confluent import ConfluentBroker
...
def get_kafka_consumer_class() -> type:
    ...
    if kafka_backend == "confluent":
        from strider.messaging.confluent import ConfluentConsumer
...
async def publish(
    ...
    from strider.messaging.topics import Topic
...
async def publish_event(
    ...
    """
    from strider.messaging.base import Event
    from strider.config import get_settings
