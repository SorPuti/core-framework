...
from strider.config import get_settings, configure
...
def get_messaging_settings():
    ...
    warnings.warn(
        ...
        "Use get_settings() from strider.config instead. "
...
def configure_messaging(**kwargs):
    ...
    Ou via código:
        from strider.config import configure
        configure(kafka_backend="confluent")
    """
    warnings.warn(
        "configure_messaging() is deprecated. "
        "Use configure() from strider.config instead, or set values in .env.",
        DeprecationWarning,
        stacklevel=2,
    )
