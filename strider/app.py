...
from strider.config import Settings, get_settings
from strider.models import create_tables, close_database
...
class StrideApp:
    ...
    Exemplo com middlewares adicionais:
        app = StrideApp(
            middleware=[
                "strider.middleware.TimingMiddleware",
                "strider.auth.AuthenticationMiddleware",
                ("strider.middleware.LoggingMiddleware", {"log_headers": True}),
            ],
        )
    ...
    def __init__(
        ...
        **fastapi_kwargs: Any,
        ...
        Middleware format (novo, Django-style):
            middleware=[
                "strider.auth.AuthenticationMiddleware",  # String path
                "auth",  # Shortcut
    ...
    async def _startup(self) -> None:
        ...
        from strider.database import init_db
        ...
        # ── Step 4: Tenancy ──
        if self.settings.tenancy_enabled:
            from strider.tenancy import set_tenant_field
    ...
    async def _auto_collect_permissions(self) -> None:
        ...
        """
        try:
            from strider.models import Model, get_session
    ...
    async def _validate_schemas(self) -> None:
        ...
        """
        import logging
        logger = logging.getLogger("strider.app")
        
        try:
            from strider.views import validate_pending_viewsets
            from strider.validation import SchemaModelMismatchError
    ...
    async def _shutdown(self) -> None:
        ...
        try:
            from strider.messaging.registry import stop_all_producers
        ...
        if self.settings.has_read_replica:
            from strider.database import close_replicas
    ...
    def _setup_tenancy_middleware(self) -> None:
        ...
        from strider.tenancy import TenantMiddleware
    ...
    def _setup_django_style_middleware(
        ...
        Args:
            middleware_list: Lista de middlewares
                - String: path do middleware (ex: "strider.auth.AuthenticationMiddleware")
    ...
    def _setup_admin(self) -> None:
        ...
        """
        try:
            from strider.admin import default_site
    ...
    def _setup_health_checks(self) -> None:
        ...
        async def readyz():
            ...
            if getattr(self.settings, "kafka_enabled", False):
                try:
                    from strider.messaging.registry import get_broker
                    broker = get_broker()
...
def get_application(
    ...
    **kwargs: Any,
    ...
    """
    from strider.config import configure, get_settings, is_configured
