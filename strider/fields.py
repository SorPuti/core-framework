...
class FieldFile:
    ...
    def save(self, filename: str, content: bytes, content_type: str | None = None) -> str:
        ...
        """
        from strider.storage import save_file
    ...
    def delete(self, save_model: bool = False) -> bool:
        ...
        from strider.storage import delete_file
...
class AdvancedField:
    """
    Namespace for UUID, JSON, and file field types.

    For slug, email, long_text prefer stride.models.Field (Field.slug(), Field.email(), Field.text()).
    AdvancedField provides: uuid_pk, uuid, json_field, long_text, slug, email, file.
    """
        ...
        *,
        ...
        Args:
            ...
            default: Default value - dict or list (default: {} for non-nullable).
                     Can also be the *type* ``dict`` or ``list``; in that case a
                     fresh instance is created on each call.
        ...
        def default_factory():
            ...
            # if user passed the type itself, instantiate it now
            if isinstance(default, type):
                if issubclass(default, (dict, list)):
                    return default()
            # for concrete instances, return a shallow copy to avoid shared state
    ...
    def struct(
        ...
        *,
        ...
        """
        Create a structured JSON field using StructSchema.
        
        Similar to json_field but with schema validation and typed access.
        
        Args:
            schema_class: StructSchema class defining the structure
            default: Default value (dict or StructSchema instance)
            nullable: Whether field can be NULL
            index: Whether to create GIN index on PostgreSQL
        
        Example:
            from strider.schema import StructSchema, StringField, BooleanField
            
            class Settings(StructSchema):
                theme = StringField(default="dark", choices=["dark", "light"])
                notifications = BooleanField(default=True)
            
            class User(Model):
                settings: Mapped[Settings] = AdvancedField.struct(Settings)
        ...
        Note:
            - Returns the column; use Field.struct() for full descriptor integration
            - For complete functionality, prefer Field.struct() in models
        """
        from strider.schema import StructSchema
